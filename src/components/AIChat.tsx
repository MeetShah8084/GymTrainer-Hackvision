      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-background-light dark:bg-background-dark relative">
              ...
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex items-end gap-3 w-full md:max-w-none max-w-[85%] ${msg.sender === 'user' ? 'justify-end ml-auto' : 'items-start justify-start'}`}>
                  ...
                  <div className={`flex flex-1 flex-col gap-1.5 ${msg.sender === 'user' ? 'items-end text-right' : 'items-start'}`}>
                    <p className={`text-[11px] md:text-[13px] font-semibold uppercase tracking-wider ${msg.sender === 'user' ? 'text-slate-400 mr-1 md:mr-0' : 'text-primary ml-1 md:ml-0'}`}>
                      {msg.sender === 'user' ? 'Me' : 'Progressive AI'}
                    </p>
                    <div className={`px-4 py-3 md:px-5 md:py-4 shadow-sm border ${
                      msg.sender === 'user' 
        ...
        >
          <div className="mx-auto flex flex-col w-full max-w-[960px]">
            {/* Expanded Greeting */}
            <div 
              className={`flex flex-col items-center justify-center transition-all duration-700 ease-in-out origin-bottom ${
                !hasStarted ? 'opacity-100 max-h-24 mb-6 translate-y-0' : 'opacity-0 max-h-0 mb-0 -translate-y-4 overflow-hidden'
              }`}
            >
